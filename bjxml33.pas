...
//    function FindElement(const anElementName, anAttrName: TString; const anAttrValue: Variant): IbjXmlElement;
    ...
    procedure LoadJS(aStream: TStream; const Encoding: String = ''); overload;
    procedure LoadJsnFile(const aFileName: String; const Encoding: String = ''); overload;
...
function CreatebjNameTable(aHashTableSize: Integer = DefaultHashSize): IbjXmlNameTable;
...
resourcestring
  ...
  SSimpleXmlError29 = 'Non-Standard Trailing Comma';
  {$ELSE}
...
//        Chr($D): begin Move(PMyXMLChar(CR)^, Result[j], 5*SizeOf(TmyXMLChar)); Inc(j, 5) end;
//        Chr($A): begin Move(PMyXMLChar(LF)^, Result[j], 5*SizeOf(TmyXMLChar)); Inc(j, 5) end;
        else begin Result[j] := aText[i]; Inc(j) end;
      end;
  end;
  for i := 1 to Length(Result) do
  if ((Result[i] = Char($C2)) and (Result[i+1] = Char($A0))) then
  begin
    Result[i] := ' ';
    Result[i+1] := ' ';
  end;
...
{$IFDEF fpc}
varOleStr:   Result := Utf8Encode(UnicodeString(TVarData(v).VOleStr));
{$ELSE}
...
function NameHashKey(aName: PByte; Count: Integer): Cardinal;{$IFNDEF fpc}{$IF CompilerVersion>=18}inline;{$IFEND}{$ENDIF}
var
  i: Integer;
begin
  Result := 0;
...
function CreatebjNameTable(aHashTableSize: Integer): IbjXmlNameTable;
begin
...
//    procedure ExpectAnyChar(aChars: string);
    ...
    procedure DropToken;
    function ExpecTbjJSNName: TmyXMLString;
...
//    function DoCloneNode(aDeep: Boolean): IbjXml; override;
    ...
    function DoShortenTree: IbjXml; virtual;
  protected
    // IbjXml
    function Get_NameTable: IbjXmlNameTable;
    function Get_NodeName: TbjXmlString;
...
//    property RestrictNames: Boolean write FRestrictNames;
  ...
  private
    ...
    function DoCloneNode(aDeep: Boolean): IbjXml; override;
    function DoShortenTree: IbjXml; override;
...
function TbjXml.GetChildNodes: IbjXmlNodeList;
begin
  Result := GetChilds
end;
...
// Verification of more chars needed for value
function NameCanContain4S(aChar: UCS4Char): Boolean;
begin
  Result := (aChar = UCS4Char('_')) or (aChar = UCS4Char('-')) or
            (aChar = UCS4Char('.')) or (aChar = UCS4Char(' ')) or
            ((aChar<=$FFFF) and IsCharAlphaNumericW(WideChar(aChar)));
...
// Verification of more chars needed for element
function NameCanContain4Q(aChar: UCS4Char): Boolean;
begin
  Result := (aChar = UCS4Char('_')) or (aChar = UCS4Char('-')) or
            (aChar = UCS4Char('.')) or
            ((aChar<=$FFFF) and IsCharAlphaNumericW(WideChar(aChar)));
...
begin
  ...
  if (aStr[rlen-1] = #10) then
  begin
    if (aStr[rlen-2] = #13) then
      SetLength(aStr, rLen-3)
    else
      SetLength(aStr, rLen-2)
  end;
  Result := aStr
end;
...
begin
{ Initialize Result }
  if DiArrItemTagID = -1 then
    DiArrItemTagID := FNames.GetID(DiArrItemTag);
  Result := '';
  rObjBegin := ': {';
  rObjEnd := '}';
  rArrBegin := ':[';
...
// Check
      ...
      Result := NL + GetIndentStr(aXMLIndent) + TextTojsoN(aTag) + ': ' + aChildsXML + ',';
    end;
      ...
      begin
        ...
        begin
          ...
          if FChilds.FItems[i].FIsArrayTag then
            Result := Result + NL + aChildsXML + NL + ','
          else
            Result := Result + NL + TextTojsoN(pTag) + ': {' +aChildsXML + NL + '},'
        end
        else
        begin
          aChildsXML := rGrandChilds[j].Get_JSN;
          if FNames.GetID(pTag) <> DiArrItemTagID then
          begin
            Result := Result + NL + GetIndentStr(aXMLIndent) + TextTojsoN(pTag) + ': ' + aChildsXML + ',';
          end
          else
            Result := Result + NL + GetIndentStr(aXMLIndent) + aChildsXML + ','
        end;
      end;
    end;
    if rGrandChilds.Count > 1 then
    begin
      ...
      begin
        ...
        begin
          ...
          aItems := aItems + NL + GetIndentStr(aXMLIndent) + TextTojsoN(aTag) + ': ' + '""' + ',';
        end;
      ...
      aChildsXML := FChilds.FItems[i].Get_JSN;
      if not ((FChilds.Get_Item(i).IsArrayTag) or (FNames.GetID(pTag) = DiArrItemTagID)) then
      begin // OBJ
//        aChildsXML := FChilds.FItems[i].Get_JSN;
        aChildsXML := RemoveTrailingComma(aChildsXML);
        Result := Result + NL + GetIndentStr(aXMLIndent) + TextTojsoN(pTag) + ': {' + aChildsXML + NL + '},';
      end
      else
...
//        aChildsXML := FChilds.FItems[i].Get_JSN;
      ...
      if FNames.GetID(pTag) = DiArrItemTagID then
        Result := Result +  GetIndentStr(aXMLIndent) + NL + '{'  + aItems + NL + '},';
    end;
  end;
  if FIsArrayTag then
  begin
    Result := RemoveTrailingComma(Result);
    Result := NL + gpTag + ': [' + Result + NL + '],';
  end;
end;
...
begin
  ...
  if Assigned(FChilds) and (FChilds.FCount > 0)
  then begin
//    pNameID := Get_NodeNameID;
    for i := 0 to FChilds.FCount - 1 do
    begin
      aNameID := FChilds.FItems[i].Get_NodeNameID;
      aChilds := FChilds.FItems[i].GetChilds;
      Exists_Item := False;
      for j := 0 to aChilds.FCount -1 do
      begin
        rGchild := TbjXmlElement(aChilds.FItems[j]);
        if (rGchild.Get_NodeNameID = DiArrItemTagID) then
        begin
          rGchild.SetNodeNameID(aNameID);
          aClone.AppendChild(rGChild.ShortenTree);
          Exists_Item := True;
        end;
      end;
      if not Exists_Item then
        aClone.AppendChild(FChilds.FItems[i].ShortenTree)
    end
  end;
end;
...
function TbjXmlText.DoShortenTree: IbjXml;
begin
  Result := TbjXmlText.Create(FNames, FData);
end;
...
function TbjXml.DoShortenTree: IbjXml;
var
  aClone: TbjXml;
  i: Integer;
begin
...
procedure TbjXml.LoadJS(aStream: TStream; const Encoding: String);
var
  aXml: TbjXmlSource;
  aBinarySign: TMyXMLString;
  aReader: TBinXmlReader;
...
begin
  ...
  try
    ...
    if DiArrItemTagID = -1 then
        DiArrItemTagID := FNames.GetID(DiArrItemTag);
    rNode.GetChilds.ParseJSN(aXml, FNames, FOnTagBegin, FOnTagEnd, '', False);
...
procedure TbjXml.LoadJsnFile(const aFileName, Encoding: String);
var
  aFile: TFileStream;
begin
  aFile := TFileStream.Create(aFileName, fmOpenRead or fmShareDenyWrite);
  try
    LoadJS(aFile, Encoding);
  finally
    aFile.Free
  end
end;
...
function TbjXmlSource.ExpecTbjJSNName: TmyXMLString;
begin
  NewToken;
  AppendTokenChar(CurChar);
  while Next and NameCanContain4Q(CurChar) do
    AppendTokenChar(CurChar);
  Result := AcceptToken;
end;
...
// Loop through all subnodes
  ...
  begin
    ...
    if aNode.NodeNameID = rNameID then
    begin
      Result := aNode;
    end;
  end;
...
//    ShowMessage(sNode.Tag + ' Entered');
    ...
    begin
      ...
      begin
        ...
        for j := 0 to rGChilds.Count - 1 do
        begin
          rGChild := rGChilds.Item[j];
          if rGChild.NumChildren > 0 then
          begin
            sNode.InsertBefore(rGChild, rChild);
            rOnce := True;
          end
        end;
        rLChild := rChild.GetNextSibling;
        if rOnce then
        begin
          sNode.RemoveChild(rChild);
          rChild := rLChild;
          Continue;
        end;
      end;
...
function TbjXml.GetArrayNodes(aName: TbjXmlString; IsIncludeOn: Boolean = True): IbjXmlNodeList;
var
  aChilds: TbjXmlNodeList;
  rNode, aNode: IbjXml;
  i: integer;
...
begin
  ...
  begin
    aNode := aChilds.Get_Item(i);
    if (aNode.NodeNameID = rNameID) and IsIncludeOn then
      rNode.AppendChild(aNode);
    if (aNode.NodeNameID <> rNameID) and (not IsIncludeOn) then
      rNode.AppendChild(aNode);
  end;
...
procedure TbjXmlNodeList.ParseJSN(aXML: TbjXmlSource; aNames: TbjXmlNameTable;
  ...
  procedure ParseElement(aMode: boolean);
  var
    aNameID: NativeInt;
    aNode: TbjXmlElement;
    rName: TbjXmlString;
